...
const NewBillScreen = ({ navigation, route }) => {
    ...
    const calculatedPreviewPrice = useMemo(() => {
        // 1. Get all inputs as clean numbers
        const grams = Number(itemGrams) || 0;
        const qty = Number(itemQuantity) || 1; // Default to 1 if empty
        ...
        // 2. Logic: Custom Rate takes priority
        if (customRateStr !== '' && customRate > 0) {
            return Math.round(customRate * qty);
        }

        // 3. Logic: Weight-based calculation
        // Formula: (entered_grams / base_weight) * base_price
        if (grams > 0 && productRate > 0) {
            const pricePerGram = productRate / baseWeight;
            const itemPrice = pricePerGram * grams;
            return Math.round(itemPrice * qty);
        }

        return 0;
    }, [itemGrams, itemQuantity, itemRate, selectedProduct]);
    ...
    return (
        ...
        >
            ...
            {/* MAIN SCROLLABLE CONTENT - Everything wraps here */}
            <ScrollView
                style={styles.mainScrollView}
                contentContainerStyle={styles.scrollContent}
                keyboardShouldPersistTaps="handled"
            >
                {/* ZONE 1: Customer Details */}
...
const styles = StyleSheet.create({
    ...
    // REMOVED COMPLEX NESTED SCROLL STYLES
